...
public class WalmartWebsiteConnect implements IWebsiteConnect {
	...
	public Double getPrice(String toyName) throws IOException {
		...
		for (Element walmartElement : document.select(".tile-content-wrapper")) {
			final String productName = walmartElement.getElementsByClass(
					"js-product-title").text();
			final String priceString = walmartElement
					.getElementsByClass("price").text().replaceAll("\\$", "");
			if (!priceString.equals("")) {
				final Double price = Double
						.parseDouble(priceString.split(" ")[0]);
				walmartItems.put(productName, price);
			}

		}

		System.out.println("The Walmart item price is :"
				+ walmartItems.get(toyName));
