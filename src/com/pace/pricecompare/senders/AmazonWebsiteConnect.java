...
public class AmazonWebsiteConnect implements IWebsiteConnect
{
	...
	{
		derivedURL = "s/ref=nb_sb_noss_2?url=search-alias%3Daps&field-keywords=" + toyName.replaceAll("\\s+", "\\+");
		...
		{
			final String name1 = element1.getElementsByClass("a-size-medium").text();
			final String priceString1 = element1.getElementsByClass("a-size-base").text().replaceAll("\\$", "");
			if (!priceString1.equals(""))
			{
				final Double price1 = Double.parseDouble(priceString1.split(" ")[0]);
